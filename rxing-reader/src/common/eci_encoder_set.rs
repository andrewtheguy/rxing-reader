...
/// Set of character encoders that can cover a given input string.
///
/// Invariants:
/// - The list contains only character sets with defined ECI values.
/// - The list contains at least one encoder for every character in the input.
/// - The first encoder in the list is always the ISO-8859-1 encoder even if no character in the input can be encoded
///   by it.
...
impl ECIEncoderSet {
    ...
    ///
    /// - `string_to_encode`: the string that needs to be encoded
    /// - `priority_charset`: preferred character set, if any.
    /// - `fnc1`: character in the input that represents FNC1 for GS1 data.
    ///   When specified, it is an error to pass it to [`Self::can_encode`],
    ///   [`Self::encode_char`], or [`Self::encode_string`].
    pub fn new(
