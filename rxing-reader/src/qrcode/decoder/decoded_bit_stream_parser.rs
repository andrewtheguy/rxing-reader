            ...
            Mode::Eci => {
                ...
                if current_character_set_eci.is_none() {
                    return Err(Error::InvalidFormat {
                        message: format!("Value of {value} not valid"),
                    }
                    .into());
                }
            }
...
fn decode_byte_segment(
    ...
    let encoding = if current_character_set_eci.is_none() {
        ...
        // give a hint.
        string_utils::guess_charset(&read_bytes, hints).ok_or(Error::InvalidState {
            message: "required internal state is missing".to_owned(),
        })?
    } else {
        current_character_set_eci.ok_or(Error::InvalidState {
            message: "required internal state is missing".to_owned(),
        })?
    };
