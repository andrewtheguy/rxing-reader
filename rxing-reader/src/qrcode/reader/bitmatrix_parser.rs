...
pub fn read_format_information(bit_matrix: &BitMatrix) -> Result<FormatInformation> {
    ...
    let mut format_info_bits1: u16 = 0;
    for x in 0..6 {
    ...
    let mut format_info_bits2: u16 = 0;
    ...
    FormatInformation::decode_qr(format_info_bits1, format_info_bits2)
}
...
pub fn read_codewords(
    ...
    let mut right_column = dimension - 1;
    while right_column > 0 {
        ...
        for row in 0..dimension {
            ...
            for col in 0..2 {
                let x = right_column - col;
                // Ignore bits covered by the function pattern
                if !function_pattern.get(x, y) {
                    // Read a bit
                    append_bit(
                        &mut current_byte,
                        format_info.data_mask.is_masked(x, y)
                            != module_bit(bit_matrix, x, y, format_info.is_mirrored),
                    );
