...
impl<T: Default + Clone + Copy> Matrix<T> {
    pub fn with_data(width: usize, height: usize, data: Vec<Option<T>>) -> Result<Matrix<T>> {
        let expected = width
            .checked_mul(height)
            .ok_or_else(|| Error::InvalidArgument { message: "invalid size: width * height overflow".to_owned() })?;
        if data.len() != expected {
            return Err(Error::InvalidArgument { message: "invalid size: data length does not match width * height".to_owned() }
            .into());
        }
        Ok(Self {
            width,
    ...
    pub fn new(width: usize, height: usize) -> Result<Matrix<T>> {
        let size = width
            .checked_mul(height)
            .ok_or_else(|| Error::InvalidArgument { message: "invalid size: width * height is too big".to_owned() })?;
        Ok(Self {
            width,
            height,
            data: vec![None; size],
